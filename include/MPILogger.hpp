...
public:
    static void prefixFormatter(std::ostream& stream, const google::LogMessage& message, void* /*data*/) {
        stream << "[Rank " << rank_ << "] "
               << google::GetLogSeverityName(message.severity())[0]
               << std::setw(2) << std::setfill('0') << message.time().month()
               << std::setw(2) << message.time().day()
               << ' '
               << std::setw(2) << message.time().hour() << ':'
               << std::setw(2) << message.time().min() << ':'
               << std::setw(2) << message.time().sec() << "."
               << std::setw(6) << message.time().usec()
               << ' '
